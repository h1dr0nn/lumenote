...
export const NoteItem = memo(({ 
    ...
    const isUnsaved = useStore(state => state.unsavedNoteIds.has(note.id));
    ...
    return (
        ...
        >
            ...
            {isEditing ? (
                ...
                <>
                    <span className="text-sm truncate font-medium flex-1">{note.title || "Ghi chú không tên"}</span>
                    {isUnsaved && (
                        <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="w-2 h-2 rounded-full bg-white opacity-80 shrink-0 shadow-sm"
